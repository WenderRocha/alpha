    ...
    <Modal v-if="isShowModal" @close="closeModal">
      <template #header>
        <div class="flex items-center text-lg">
          ...
          <div class="text-sm">{{ assetsForm.asset1.toUpperCase() }} <span class="uppercase">{{ (assetsForm.asset1 !=
              '') ? 'x' : ''
          }}</span> {{ assetsForm.asset2.toUpperCase() }}</div>
        </div>
      </template>
      <template #body>
    ...
    <Modal size="md" v-if="isShowModalDelete" @close="closeModalDelete">
      <template #body>
        <div class="p-6 text-center">
          ...
          <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Deseja realmente excluir o ativo?</h3>
